    ...
    if ((isPrivate || isMention || inConversation) && !triggers.length) {
      if (self.triggers['{DM}']) {
        if (typeof self.triggers['{DM}'] === 'function') {
          return self.triggers['{DM}'](text, function(err, reply) {
        ...
        var reply = self.ai.reply({
          name: 'defaultname1' || self.userMap[message.user].name || 'defaultname'
        }, text, function(err, reply) {
...
Snarl.prototype.start = function() {
  ...
  aiml.parseDir('./data/aiml', function(err, topics) {
  //aiml.parse('./data/aiml/alice.aiml', function(err, topics) {
    console.log('err:', err);
    slack.ai = new aiml.AiEngine('Maki Slack', topics, { name: 'snarl' });
